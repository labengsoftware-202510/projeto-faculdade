...
import Page.Page_Pessoas_C as Pg_C
import Page.Page_Pessoas_U as Pg_U
import Page.Page_Pessoas_D as Pg_D
...
def pessoasR(filtros):
    ...
    if tabela[0]:
        ...
        rowSel = st.dataframe(data=dtFrame,
                            ...
                            on_select='rerun',
                            column_config=cc.colConfigPessoas())
        ...
        if selRow:
            selIndex = {'reg_ins': dtFrame['reg_ins'].iloc[selRow].item(),
                        ...
                        'cpf_format': dtFrame['cpf_format'].iloc[selRow].item(),
    ...
    if regButton:
        Pg_C.insPessoas()
    if altButton:
        Pg_U.altPessoas(selIndex)
    if delButton:
        Pg_D.delPessoas(selIndex)